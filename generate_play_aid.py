...
def generate_play_aid(output_file="PLAY_AID.md"):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("  - **After each non-TD drive:** Roll for yards to go (style-dependent)\n")
        ...
        f.write("  - **If yards to go ≥ 11:** Automatic failure (can't convert 11+ yards)\n")
        ...
        f.write("  - **If going for it:** Roll d10 for attempt\n")
        f.write("    - **Success if roll > yards to go:** Move ball forward by roll amount\n")
        f.write("    - **If reaches goal line:** Touchdown (7 points)\n")
        f.write("    - **Otherwise:** Same team starts fresh drive from new position\n")
        f.write("    - **If unsuccessful:** Turnover on downs (opponent gets ball)\n\n")
